...
Lately, I've been sharing more and more about what I learned when building an AI architecture for a classic AI chat agent (like we did with Charlie). 
...
Consider this a clean introduction to the core pillars, stripped of all the unnecessary bells and jingles. 
...
I strongly discourage creating this layer from scratch,it makes sense for very simple, single-turn flows. 
...
There are solid alternatives on the rise, [PydanticAI](https://pydantic.dev/docs/ai/overview/) is gaining traction because it promises framework stability, and [Vercel AI SDK](https://ai-sdk.dev/) is quite popular too... But here is a small tip: if you aren't terrified of vendor lock-in, just use one of the main provider libraries. They share more or less the same feature, and if you've already committed to a cloud vendor, using their native SDK is usually the lowest-friction route.
...
Given the way these orhctrator are built, if you are careful enough and keep it lightweight, you shouldn't be locked in enough. But choosing the right SDK to get started withll simplify your life a lot.
...
A crucial tip for the fast model: avoid giving it a long thinking budget. You do not want your AI to sit there running reasoning loops for 30 seconds just to say "hello" to a user.

As you know my style, let's keep thing simple. Keep your agent network tight. 
...
Still for the sake of this generic post, 2026-era models are smart enough to work perfectly well with simple, direct instructions.

I wouldn't stress over-engineering this part because it depends entirely on your codebase, but 3 key points are:
...
See them as skills, [plugins, MCPs, whatever you want to call them](https://mutt0-ds.github.io/posts/2024/06/ai-superpowers-tools/), the concept is functions, code that it can run to fetch data to provide a better response.
...
I really want to emphasize the word "*SIMPLE*". If you write a 50-line instruction block for a specific tool, *that entire block gets injected into the general system prompt context on every single startup turn*. This is 1. slow, 2. expensive and 3. unncessary, especially for a niche tool that only gets called 5% of the time.
...
A pretty solid guardrail layer look like this:

- A foundational instruction at the root level (e.g., "You only talk about Mathematics"). This offers a basic baseline of defense, and modern models are already quite decent at self-defense if an exploit somehow slips past outer layers.
...
Also, pay close attention to custom fields inside your tools. If you allow unfiltered, raw text into tool arguments without strict schema enforcement and sanitation, you are opening your application up to easy injections. We could say much more about this topic, but you should have catch the key points for now: never trust the AI in defending itself from attacks.

## Evals

To me, this is an absolute pillar, completely non-negotiable, just like unit testing for traditional applications. **You need a resilient evaluation suite**.
...
This ties directly into eval-driven development. Start from a defined goal: you usually don't want an AI agent that attempts to do literally everything; you want it bounded to a few core tasks. 
...
Wow, that went fast. 
We've only scratched the surface, but here are a few final infrastructural points I would always include in a production-ready Conversational AI setup:

- The Configuration: in production, you also need the agility to deploy an emergency prompt hot-fix instantly without waiting for a full 10-minute CI/CD deployment pipeline. Store your base prompts, model settings, and configurations in a database that can be synced to the orchestrator
- A solid Observability Layer: you absolutely need deep tracing for debugging when something goes wrong, and trace hallucinations, spending and other KPI. There are plenty of great production solutions out there, many of which are completely self-hostable
- Graceful Fallbacks: LLMs fail, rate limits get choked, and cloud vendor APIs go down. You should always have a backup model hosted by an entirely different provider ready to take over if your main provider becomes unresponsive—it's the only way to ensure high uptime
...
Those were the pillars for a good conversational AI architecture. Something that I affined over the years, building different projects in different areas.
I know we just had a very quick overview of a very vast, complex and rapidly-evvoling world, but I hope this will be helpufl! I was looking for a post like this before creating [Charlie](https://mutt0-ds.github.io/posts/2026/04/charlie-lessons/).
If you have ny question or want to explore more in-depth a point, [you know where to find me](https://www.linkedin.com/in/davide-muttoni-77b134194/).
